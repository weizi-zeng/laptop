...
require(ROOT_PATH . 'languages/common.php');
require(ROOT_PATH . 'languages/log_action.php');

if (file_exists(ROOT_PATH . 'languages/' . basename(PHP_SELF)))
{
    include(ROOT_PATH . 'languages/' . basename(PHP_SELF));
}
...
$smarty->assign('ADMIN_PATH', ROOT_PATH . ADMIN_PATH."/");
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
        ...
        $sql = 'SELECT restaurant, user_id, user_name, password, action_list, last_login ' .
        ...
        else
        {
            ...
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['admin_pass'])
            {
                ...
                set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['last_time'], $row['restaurant']);
