...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    {
         /* 检查密码是否正确 */
         $sql = "SELECT restaurant, user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
    ...
    if ($row)
    {
        ...
        // 登录成功
        set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['last_login'], $row['restaurant']);
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!empty($_POST['user_name']))
    {
        ...
        if (!$is_only)
        {
            sys_msg(sprintf("此缩写已经存在！", stripslashes($_POST['user_name'])), 1);
        }
    }
    ...
    if (!empty($_POST['email']))
    {
        ...
        if (!$is_only)
        {
            sys_msg(sprintf("此邮箱已经被注册！", stripslashes($_POST['email'])), 1);
        }
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (restaurant, user_name, email, password, add_time, nav_list, action_list, role_id) ".
           "VALUES ('".trim($_POST['restaurant'])."','".trim($_POST['user_name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id')";
     ...
     * 菜品类型表
     */
    create_table(trim($_POST['user_name']));
...
//     $link[0]['href'] = 'privilege.php?act=allot&id='.$new_id.'&user='.$_POST['user_name'].'';
    ...
    sys_msg($_LANG['add'] . "&nbsp;" .$_POST['user_name'] . "&nbsp;" . "成功！",0, $link);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if ($_REQUEST['id'] == 1)
    {
    	...
    	sys_msg("laptop 为最高管理员，不能进行编辑！", 0, $link);
    }
    ...
    /* 获取管理员信息 */
    $sql = "SELECT restaurant, user_id, user_name, email, password, agency_id, role_id FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    if (!empty($admin_email))
    {
        ...
        if ($is_only == 1)
        {
            sys_msg(sprintf("此邮箱邮件存在！", stripslashes($admin_email)), 1);
        }
    }
...
//     }
   ...
   /* 提示信息 */
   $link[] = array('text' => strpos($g_link, 'list') ? "返回列表" : "修改信息", 'href'=>$g_link);
...
{
    include_once(ROOT_PATH . 'languages/priv_action.php');
...
{
    $list = array();
    $sql  = 'SELECT user_id, restaurant, user_name, email, add_time, last_login '.
